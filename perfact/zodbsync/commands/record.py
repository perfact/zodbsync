...
class Record(SubCommand):
    ...
    def commit(self):
        ...
        if codechg_mail:  # pragma: no cover
            ...
            msg["From"] = self.config.get("codechange_sender", "codechanges@perfact.de")
        ...
        if self.args.autoreset:
            reset = Reset(sync=self.sync, logger=self.logger, config=self.config)
    ...
    def run(self):
        ...
        if self.args.lasttxn:
            ...
            if res["search_limit_reached"] or res["limit_reached"] or res["no_records"]:
        ...
        self.sync.record(
            paths=paths, recurse=recurse, skip_errors=self.args.skip_errors
        )
