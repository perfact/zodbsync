...
class SubCommand(Namespace):
    ...
    def gitcmd_output(self, *args):
        ...
        return subprocess.check_output(self.gitcmd(*args), universal_newlines=True)
    ...
    def datafs_filesystem_path(self, path):
        ...
        if path.startswith(self.sync.site):
            filesystem_path = os.path.join(self.config["base_dir"], path)
        else:
            filesystem_path = os.path.join(
                self.config["base_dir"], self.sync.site, path
            )
    ...
    def _branch_info(self):
        ...
        points."""
        current = self.gitcmd_output("rev-parse", "--abbrev-ref", "HEAD").strip()
    ...
    def check_repo(self):
        ...
        self.unstaged_changes = [
            line[3:]
            for line in self.gitcmd_output("status", "--untracked-files", "-z").split(
                "\0"
            )
            if line
        ]
    ...
    def _playback_paths(self, paths):
        ...
        for ix, phase in enumerate(phases):
            ...
            self.logger.info("Calling phase %s, command: %s", phase_name, phase_cmd)
            proc = subprocess.Popen(
                phase_cmd,
                stdin=subprocess.PIPE,
                stdout=subprocess.PIPE,
            ...
            out, _ = proc.communicate(json.dumps({"paths": phase["paths"]}))
            returncode = proc.returncode

            if returncode:
                self.logger.error("Error during phase command %s, %s", returncode, out)
                if sys.stdin.isatty():
                ...
                raise AssertionError("Unrecoverable error in phase command")
            else:
        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if os.path.exists(cpfname):
                    ...
                    try:
                        self.gitcmd_run(
                            "diff", failed_commit + "~", "HEAD", "--", *affected_files
                        )
