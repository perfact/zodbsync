...
def mod_read(
    obj=None, onerrorstop=False, default_owner=None, force_default_owner=False
):
...
def mod_write(
    ...
    if obj is not None and not hasattr(obj, "meta_type"):
        logging.getLogger("ZODBSync").warning(
            "Removing property with colliding ID! ({} in {})".format(obj_id, parent)
        )
...
class ZODBSync:
    ...
    def fs_write(self, path, data):
        ...
        meta = {key: value for key, value in data.items() if key not in exclude_keys}
        fmt = mod_format(meta)
        ...
        if old_data != new_data:
            ...
            source_files = [
                s
                for s in os.listdir(write_base)
    ...
    def fs_parse(self, fspath, data=None):
        ...
        if len(src_fnames) > 1:
            raise AssertionError(f"Multiple source files in {fspath}")
    ...
    def record(self, paths, recurse=True, skip_errors=False, ignore_removed=False):
    ...
    def record_obj(self, obj, path, recurse=True, skip_errors=False):
        ...
        if now - self.num_obj_last_report > 2:
            self.logger.info(
                "%d obj saved of at least %d, "
                "current path %s" % (self.num_obj_current, self.num_obj_total, path)
            )
    ...
    def _playback_path(self, pathinfo):
        ...
        if self.recurse:
            ...
            del_ids = [
                a
                for a in srv_contents
                if a not in contents and not (obj == self.app and a == "acl_users")
            ]
            if del_ids:
    ...
    def prepare_paths(self, paths):
        ...
        paths = {
            (
                path.rsplit("/", 1)[0]
                if (path.rsplit("/", 1)[-1].startswith("__"))
                else path
            )
            for path in paths
        }
    ...
    def playback_paths(
        self, paths, recurse=True, override=False, skip_errors=False, dryrun=False
    ):
