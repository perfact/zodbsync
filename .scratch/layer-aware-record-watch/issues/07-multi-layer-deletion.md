...
Current behaviour: when an object is deleted from Zope and it exists in a lower named layer, a `__deleted__` marker is always placed in the fallback layer (ident `""`) to shadow it.
...
- [x] Deleting an object that exists in only one named layer removes the files without creating `__deleted__`.
- [x] Deleting an object that exists in the fallback layer only removes the files without creating `__deleted__` (single-layer case).
- [x] Deleting an object that exists in two layers places `__deleted__` in the topmost layer.
- [x] Existing tests for deletion behaviour pass.
- [x] New tests cover both the single-layer and multi-layer deletion cases.
