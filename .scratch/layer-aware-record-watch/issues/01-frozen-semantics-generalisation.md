...
- [x] `fs_pathinfo` scans all layer workdirs for `__frozen__` at each path component (not only `base_dir`).
- [x] `__frozen__` found in layer at index N restricts to `layers[:N+1]`.
- [x] All existing tests that rely on `__frozen__` in the custom layer continue to pass unchanged.
- [x] New test: `__frozen__` placed in a named layer's workdir causes layers below it to be ignored for that path and descendants.
- [x] New test: `__frozen__` in a named layer does not affect paths outside that subtree.
